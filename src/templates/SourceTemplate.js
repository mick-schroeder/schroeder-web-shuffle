...
const SourceTemplate = ({ data, pageContext }) => {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      if (e.keyCode === 37 && prevSlug) { // 37 is the key code for the left arrow key
        navigate(`/sources/${prevSlug}`);
      } else if (e.keyCode === 39 && nextSlug) { // 39 is the key code for the right arrow key
  ...
  <div className="flex py-6 justify-between">
    <Link 
        ...
        onClick={(e) => { if (!prevSlug) e.preventDefault(); }}
        className={`flex items-center justify-center px-4 h-10 mr-3 text-base font-medium ${prevSlug ? 'text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white' : 'text-gray-300 bg-gray-200 border border-gray-300 cursor-not-allowed dark:bg-gray-700 dark:border-gray-600 dark:text-gray-500'}`}
    >
    ...
    <Link 
        ...
        onClick={(e) => { if (!nextSlug) e.preventDefault(); }}
        className={`flex items-center justify-center px-4 h-10 text-base font-medium ${nextSlug ? 'text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white' : 'text-gray-300 bg-gray-200 border border-gray-300 cursor-not-allowed dark:bg-gray-700 dark:border-gray-600 dark:text-gray-500'}`}
    >
        Next
